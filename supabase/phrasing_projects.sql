...
-- Shared private account profile. Username is a display/account field only;
-- Supabase email remains the sign-in and password-recovery identity.
create table if not exists public.user_profiles (
  user_id uuid primary key references auth.users(id) on delete cascade,
  username text not null
    check (
      username = lower(username)
      and username ~ '^[a-z0-9][a-z0-9_]{1,22}[a-z0-9]$'
    ),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

-- An expression index keeps uniqueness case-insensitive even if an
-- administrator imports legacy mixed-case data outside the normal client flow.
...
-- Profiles are private in this release: there is no username lookup or list.
